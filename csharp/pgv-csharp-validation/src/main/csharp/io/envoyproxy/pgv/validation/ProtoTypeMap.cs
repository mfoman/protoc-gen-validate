 ...
 */
public sealed class ProtoTypeMap
{

    private readonly ImmutableMap<String, String> types;
    ...
    public static ProtoTypeMap of(Collection<DescriptorProtos.FileDescriptorProto> fileDescriptorProtos)
    {
        ...
        const ImmutableMap.Builder<String, String> types = ImmutableMap.builder();

        foreach (DescriptorProtos.FileDescriptorProto fileDescriptor in fileDescriptorProtos)
        {
            const DescriptorProtos.FileOptions fileOptions = fileDescriptor.getOptions();

            const String protoPackage = fileDescriptor.hasPackage() ? "." + fileDescriptor.getPackage() : "";
            const String javaPackage = Strings.emptyToNull(
                    fileOptions.hasJavaPackage() ?
                            fileOptions.getJavaPackage() :
                            fileDescriptor.getPackage());
            const String enclosingClassName =
